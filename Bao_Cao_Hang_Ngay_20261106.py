...
REQUIRED_FILES = {
    ...
    "Timer_KhongNguoi": EXCEL_TIMER_KHONG_NGUOI_FILE,
    "PPT": PPT_FILE,
}
...
PPT_ANCHORS = {
    ...
    "timer_khong_nguoi": {
        "slide": 19,
        "chart": "anchor_timer_khong_nguoi"
    }
}
BLOCK_SLIDE_MAP = {
    "AA": 4,   # Cắt vật liệu
...
def load_timer_summary_data(file_path, sheet_name):
    raw = pd.read_excel(
        file_path,
    ...
    def find_row_regex(pattern):
        ...
        if not idx.any():
            raise ValueError(f"❌ Không tìm thấy dòng regex: {pattern} trong {file_path.name}")
        return idx.idxmax()
...
def load_tong_hop_timer_data():
    return load_timer_summary_data(EXCEL_TIMER_FILE, EXCEL_TIMER_SHEET)
...
def process_timer_utilization_chart(prs):
    ...
    # Thay vì load từ sheet Tong_Hop, ta tổng hợp trực tiếp từ các sheet lẻ (cột B:F)
    summary_rows = []
    for sheet_name in TIMER_SHEETS:
        try:
            # Đọc cột B:F: Ngày(B), MT(C), Có tải(D), Không tải(E), Tắt máy(F)
            ...
            if total > 0:
                summary_rows.append({
                    "Hạng mục": sheet_name,
                    "% Có tải": s_co / total,
                    "% Không tải": s_khong / total,
                    "% Tắt máy": s_tat / total,
                    "% Mục tiêu": 0  # Sẽ map từ hardcoded_targets bên dưới
                })
        except Exception as e:
            print(f"⚠️ Lỗi tổng hợp dữ liệu từ sheet {sheet_name}: {e}")

    df = pd.DataFrame(summary_rows)

    # Dữ liệu mục tiêu được nhập sẵn trong code theo yêu cầu của bạn
    # Cấu trúc: "Tên công đoạn": giá trị (ví dụ 0.8 tương ứng 80%)
    hardcoded_targets = {
        "LA": 0.21, "MA": 0.30, "BJ": 0.30, "GC": 0.35, "GS": 0.35, 
        "GR": 0.50, "EN": 0.60, "GJ": 0.50, "MC": 0.65, "LN": 0.65, 
        "EW": 0.68, "GP": 0.70
    }

    # Chuẩn hóa dữ liệu về dạng thập phân (0.xx) để khớp với logic nhân 100 khi vẽ
    for col in ["% Có tải", "% Không tải", "% Tắt máy", "% Mục tiêu"]:
        df[col] = df[col].apply(lambda x: x/100 if x > 1 else x)

    df["% Mục tiêu"] = df["Hạng mục"].map(hardcoded_targets).fillna(df["% Mục tiêu"])
...
def build_timer_khong_nguoi_df(df_block):
    """
    Xây dựng dataframe cho Timer Không Người từ sheet trend.
    Lấy dữ liệu theo các dòng cụ thể được yêu cầu.
    """
    # 1. Tìm dòng ngày làm việc để phân tách Tháng/Ngày
    i_date = find_row(df_block, ROW_DATE)
    raw_values = df_block.loc[i_date, 1:].values
    ...
    for i, v in enumerate(raw_values):
        # Nhận diện cột tháng (vd: 1月 hoặc Tháng 1)
        if isinstance(v, str) and ("月" in v or v.strip().lower().startswith("tháng")):
            month_idx.append(i)
            lbl = v.replace("月", "").replace("Tháng", "").strip()
            raw_labels.append(f"T{lbl}")
        else:
    ...
    def get_clean_row(row_name):
        idx = find_row_exact(df_block, row_name)
        s = df_block.loc[idx, 1:].astype(str).str.replace("%", "", regex=False).str.replace(",", ".", regex=False)
        return pd.to_numeric(s, errors="coerce").fillna(0).values

    # Dữ liệu Tháng
    row_mt_month = "Mục tiêu hoạt động máy không người (%/Tháng)"
    row_tt_month = "Thực tích hoạt động máy không người (%/Tháng)"
    
    vals_mt_m = get_clean_row(row_mt_month)
    vals_tt_m = get_clean_row(row_tt_month)

    df_month = pd.DataFrame({
        "Label": [raw_labels[i] for i in month_idx],
        "Mục tiêu": vals_mt_m[month_idx],
        "Thực tích": vals_tt_m[month_idx],
    })
    df_month["X"] = range(len(df_month))

    # Dữ liệu Ngày
    row_mt_day = "Mục tiêu hoạt động máy không người (%/Ngày)"
    row_tt_chinh_thuc = "Thực tích TGGC không người: Giờ làm việc chính thức (%)"
    row_tt_an_com = "Thực tích TGGC không người: Ăn cơm (%)"
    row_tt_sau_2230 = "Thực tích TGGC không người: Sau 22h30 (%)"

    vals_mt_d = get_clean_row(row_mt_day)
    vals_tt1 = get_clean_row(row_tt_chinh_thuc)
    vals_tt2 = get_clean_row(row_tt_an_com)
    vals_tt3 = get_clean_row(row_tt_sau_2230)

    df_daily = pd.DataFrame({
        "Label": [raw_labels[i] for i in day_idx],
        "Mục tiêu": vals_mt_d[day_idx],
        "TT_ChinhThuc": vals_tt1[day_idx],
        "TT_AnCom": vals_tt2[day_idx],
        "TT_Sau22h30": vals_tt3[day_idx],
    })
...
def draw_timer_khong_nguoi_combo_chart(df_daily, df_month, title, output_file, show_title=True):
    import numpy as np
    import matplotlib.pyplot as plt

    # =========================
    # ✅ NORMALIZE CHUẨN – KHÔNG PHÁ DATA
    # =========================
    ...
    # =========================
    if not df_month.empty:
        mt_month = normalize_percent(df_month["Mục tiêu"])
        tt_month = normalize_percent(df_month["Thực tích"])

        bars_m = ax_left.bar(
            df_month["X"], tt_month,
            width=width, color="#1f77b4",
            label="Thực tích hoạt động máy không người (Tháng)"
        )

        ax_left.plot(
            df_month["X"], mt_month,
            color="red", marker="o", linewidth=2.5,
            label="Mục tiêu hoạt động máy không người (Tháng)"
        )

        for b, v in zip(bars_m, tt_month):
            if v > 0:
                ax_left.text(
                    b.get_x()+b.get_width()/2,
                    v/2, f"{v:.1f}%",
        ...
        # for x_val, y_val in zip(df_month["X"], mt_month):
        #     if y_val > 0:
        #         ax_left.text(
        #             x_val,
        #             y_val + max(mt_month)*0.04,
        #             f"{y_val:.0f}%",
        #             ha="center", va="bottom",
        #             color="red", fontsize=10
        #         )

        ax_left.set_ylim(0, max(1, max(tt_month.max(), mt_month.max())*1.2))
    ...
    if not df_daily.empty:
        ...
        co = normalize_percent(df_daily["TT_ChinhThuc"])
        an = normalize_percent(df_daily["TT_AnCom"])
        ...
        # STACK BAR
        # =========================
        ax_right.bar(x_d, co, width=width, color="#2E7D32", label="Thực tích TGGC không người: Giờ làm việc chính thức (Ngày)")
        ...
        ax_right.bar(
            ...
            width=width,
            color="#FBC02D",
            label="Thực tích TGGC không người: Ăn cơm (Ngày)"
        )
        ...
        ax_right.bar(
            ...
            width=width,
            color="#9E9E9E",
            label="Thực tích TGGC không người: Sau 22h30 (Ngày)"
        )
        ...
        for i in range(len(co)):
            ...
            if co[i] > 0:
                ax_right.text(x_d.iloc[i], co[i]/2,
                              f"{co[i]:.1f}%",
                              ha="center", va="center", fontsize=8,
                              bbox=BAR_TEXT_BOX)

            if an[i] > 0:
                ax_right.text(x_d.iloc[i], co[i] + an_plot[i]/2,
                              f"{an[i]:.1f}%",
                              ha="center", va="center", fontsize=8,
                              bbox=BAR_TEXT_BOX)

            if sau[i] > 0:
                ax_right.text(x_d.iloc[i], co[i] + an_plot[i] + sau[i]/2,
                              f"{sau[i]:.1f}%",
                              ha="center", va="center", fontsize=8,
                              bbox=BAR_TEXT_BOX)

        # ✅ SCALE ĐÚNG CHO CỘT STACK
        total_day = co + an_plot + sau
        y_max = max(total_day.max() if len(total_day) > 0 else 0, mt_day_plot.max() if len(mt_day_plot) > 0 else 0)
        ax_right.set_ylim(0, max(1, y_max * 1.3))

    # =========================
    # AXIS & STYLE
    # =========================
    ax_left.set_ylabel("Tỷ lệ % (Tháng)")
    ax_right.set_ylabel("Tỷ lệ % (Ngày)")

    labels = df_month["Label"].tolist() + df_daily["Label"].tolist()
    pos = np.arange(len(labels))

    STEP = max(1, len(labels)//20)
    ax_left.set_xticks(pos[::STEP])
    ax_left.set_xticklabels(labels[::STEP], fontsize=9)
    ...
    plt.subplots_adjust(left=0.045, right=0.94, top=0.95, bottom=0.12)
...
def process_timer_khong_nguoi_chart(prs):
    ...
    """
    try:
        # Đọc dữ liệu thô từ sheet KhongNguoi
        df_raw_kn = pd.read_excel(
            EXCEL_TIMER_KHONG_NGUOI_FILE, 
            sheet_name=EXCEL_TIMER_KHONG_NGUOI_SHEET, 
            header=None
        )
        # Xây dựng dataframe theo cấu trúc Tháng/Ngày
        df_daily, df_month = build_timer_khong_nguoi_df(df_raw_kn)
    except Exception as e:
        print(f"⚠️ Bỏ qua Timer không người vì lỗi: {e}")
        return

    img_path_local = LOCAL_OUT_DIR / "timer_khong_nguoi_trend.png"

    slide_idx = PPT_ANCHORS["timer_khong_nguoi"]["slide"]

    # Cập nhật tiêu đề trên Slide tương tự slide 0
    update_title_in_ppt(
        prs,
        slide_idx,
        "BIỂU ĐỒ TỈ LỆ HOẠT ĐỘNG MÁY KHÔNG NGƯỜI NĂM 2026",
        title_shape_name="title_timer_khong_nguoi",
        font_name="Calibri",
        font_size_pt=20,
        bold=True,
        color_rgb=(0, 0, 128)
    ...
    draw_timer_khong_nguoi_combo_chart(
        df_daily, 
        df_month,
        "BIỂU ĐỒ TỈ LỆ HOẠT ĐỘNG MÁY KHÔNG NGƯỜI NĂM 2026",
        img_path_local,
        show_title=False  # Không vẽ title vào hình vì đã dùng textbox PPT
    )
    ...
    replace_image_in_ppt(
        prs,
        PPT_ANCHORS["timer_khong_nguoi"]["chart"],
        img_path_local,
        slide_index=PPT_ANCHORS["timer_khong_nguoi"]["slide"]
    )
...
def main():
    ...
    process_timer_khong_nguoi_chart(prs)
