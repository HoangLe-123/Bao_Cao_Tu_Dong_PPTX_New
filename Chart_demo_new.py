...
def save_figure(fig, output_file):
    ...
    plt.close(fig)
# ========== TUNABLES ==========
EXCEL_FILE = Path(r"Hằng ngày_data.xlsx")
EXCEL_SHEET = "Hằng ngày"
EXCEL_YEAR_FILE = Path(r"Tính sản xuất năm 2026.xlsx")
EXCEL_YEAR_SHEET = "NĂM 2026"
EXCEL_THUCTICH_FILE = Path(r"ThucTich_Data.xlsx")
EXCEL_THUCTICH_SHEET = "Sheet1"
EXCEL_COSTDOWN_FILE = Path(r"Costdown nội tác 2026.xlsx")
EXCEL_COSTDOWN_SHEET = "Quản lý MT năm"
EXCEL_TIMER_FILE = Path(r"DulieutudongTimer.xlsx")
EXCEL_TIMER_SHEET = "Tong_Hop"
PPT_FILE = Path(r"Bao_Cao_Hang_Ngay.pptx")
...
def draw_combo_chart_global(df, title, ylabel, output_file, legend_prefix=""):
    ...
    for bar in bars:
        ...
        if h > 0:
            ax.text(
                ...
                h + (ax.get_ylim()[1] * 0.01),  # ⬅️ đẩy nhẹ lên trên cột
                f"{int(h)}",
                ha="center",
                va="bottom",
                fontsize=9,
    ...
    BASE_UP = ymax * 0.035     # offset chuẩn
    BASE_DOWN = ymax * 0.04
    EXTRA = ymax * 0.045        # offset tăng cường khi 2 line quá gần
    ...
    for x_val, y1, y2 in zip(df["X"], df["LINE1"], df["LINE2"]):
        ...
        if y1 > 0:
            if delta < ymax * 0.05:
                y_text_1 = y1 - BASE_UP - EXTRA   # 🔽 đẩy thấp hơn nữa
            else:
                y_text_1 = y1 - BASE_UP
        ...
        if y2 > 0:
            if delta < ymax * 0.05:
                y_text_2 = y2 + BASE_DOWN + EXTRA  # 🔼 đẩy cao hơn nữa
            else:
                y_text_2 = y2 + BASE_DOWN
...
def draw_timer_block_chart_manual_style(df, title, output_file):
    ...
    STEP = max(1, len(df) // 10)   # ~10 label là đẹp
    # fig, ax = plt.subplots(figsize=(15, 4.8))
    fig, ax = plt.subplots(figsize=(13,2.5))   # ≈ (12.6, 3.54)
    ...
    ax.bar(x, co_tai, width=bar_width, color="#92D050", label="% Có tải")
    ax.bar(x, khong_tai, width=bar_width,  bottom=co_tai, color="#FFC000", label="% Không tải")
    ...
    if SHOW_TEXT:
        for i, v in enumerate(co_tai):
            if v > 0: #chỉ hiển thị khi khác 0%
                ax.text(
                    x[i],
                    v / 2,
                    f"{int(v)}",
    ...
    if target_c1 is not None:
        ax.axhline(target_c1, color="#C000FF", linewidth=2, label="Mục tiêu C1")

    if target_c2 is not None:
        ax.axhline(target_c2, color="red", linewidth=2, label="Mục tiêu C2")

    ax.set_ylim(0, 100)
    ax.set_xticks(x[::STEP])
    ax.set_xticklabels(df["Thời gian"].iloc[::STEP], fontsize=11)
    ax.set_ylabel("Tỷ lệ (%)", rotation=0, fontsize=11)
    ax.yaxis.set_label_coords(-0.005, 1.035)
    fig.suptitle(title, fontsize=14, y=1.09)
    # ax.set_ylabel("Tỷ lệ (%)")
    # ax.set_title(title, fontsize=14, fontweight="bold")

    ax.grid(axis="y", linestyle="--", alpha=0.25)
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=5, frameon=False, fontsize=11)
...
def draw_year_bv_pcs_combo_chart(df, title, output_file,show_title=True):
    ...
    # COLOR SYSTEM
    # ======================
    COL_BV_TARGET = "#A6CEE3"   # Mục tiêu Bản vẽ
    COL_BV_ACTUAL = "#1f77b4"   # Thực tích Bản vẽ
    COL_PCS_TARGET = "#F39C12"  # Mục tiêu PCS
    COL_PCS_ACTUAL = "#8E1E72"  # Thực tích PCS (phụ)
    ...
    # DATA LABEL – PCS
    # ======================
    # Offset theo trục Y phải (an toàn với mọi scale)
    y_pcs_max = ax2.get_ylim()[1]
    offset = y_pcs_max * 0.015   # ~1.5% trục Y, rất vừa mắt

    # ----- MỤC TIÊU PCS: LABEL NẰM TRÊN LINE -----
    for i, y in enumerate(df["MT PCS"]):
        if y > 0:
            ax2.text(
                x[i],
                y + offset,            # ⬆️ trên line
                f"{int(y)}",
                ha="center",
                va="bottom",
                fontsize=9,
    ...
    # ----- THỰC TÍCH PCS: LABEL NẰM DƯỚI LINE -----
    for i, y in enumerate(df["TT PCS"]):
        if y > 0:
            ax2.text(
                x[i],
                y - offset,            # ⬇️ dưới line
                f"{int(y)}",
                ha="center",
                va="top",
                fontsize=9,
...
def load_file3_G1_L16():
    df = pd.read_excel(
        EXCEL_THUCTICH_FILE,   # ✅ BIẾN PATH ĐÚNG
        sheet_name=0,
        usecols="G:L",
        skiprows=2,
        nrows=13
